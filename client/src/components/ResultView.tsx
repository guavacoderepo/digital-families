...
function verdict(health: number): { headline: string; caption: string } {
  if (health > 0.78) {
    return {
      ...
      caption: "You are living lightly. There is very little here that needs changing.",
    };
  }
  if (health > 0.58) {
    return {
      headline: "Your grass is doing well",
      caption: "You are below what most homes use. A couple of small changes would help further.",
    };
  }
  if (health > 0.40) {
    return {
      headline: "Your grass is starting to dry out",
      caption: "You are around the same as most homes in the UK. There is room to bring this down.",
    };
  }
  if (health > 0.22) {
    return {
      headline: "Your grass is struggling",
      caption: "Your home uses more than most. The suggestions below are the ones that matter most.",
    };
  }
  return {
    headline: "Your grass is dying back",
    caption: "Your home uses a good deal more than most. Start with the first suggestion below.",
  };
}
...
export function ResultView({ assessment, methodologyNote, onRestart }: Props) {
  ...
  const nothingAnswered = Object.values(assessment.answers).every((value) => !value);
  const ranked = [...assessment.categories].filter((c) => c.kg > 0).sort((a, b) => b.kg - a.kg);
    ...
    <div className="result">
      ...
      <section className="result__figure" aria-label="Your total">
        ...
        <p className="result__total">
          <span className="result__total-value">{number(assessment.totalKg)}</span>
          <span className="result__total-unit">kg of carbon a year</span>
        </p>
        <p className="result__lead">
          That is {kg(assessment.perPersonKg)} each. A typical person in the UK is
          around 6,500 kg.
        </p>
      </section>
      ...
      <section className="advice" aria-label="What would help most">
        ...
        <ol className="advice__list">
          {assessment.advice.lines.map((line, index) => (
      ...
      <details className="breakdown">
        <summary className="breakdown__toggle">Show where it comes from</summary>
